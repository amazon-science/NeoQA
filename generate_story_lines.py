...
Usage:
  generate-story-lines.py redo-art
  generate-story-lines.py file <file> [<genre>]
...
from src.mglockne_story_line.llm.modules.history_parsable_module_list import HistoryParsableModuleList
from src.mglockne_story_line.llm.modules.module_pipeline import ModulePipeline
...
from src.mglockne_story_line.story.event_sequence.event_sequence2 import EventSequence2
from src.mglockne_story_line.story.event_sequence.modules.fictive_entities.adjust_outline_with_entity_names import \
    ResolveFoundNamedEntityConflictsInOutlineModule
from src.mglockne_story_line.story.event_sequence.modules.fictive_entities.entity_fill_fields import PopulateNewNamedEntitiesModule
from src.mglockne_story_line.story.event_sequence.modules.fictive_entities.entity_find_names import IdentifyNewNamedEntitiesModule
from src.mglockne_story_line.story.event_sequence.modules.fictive_entities.entity_fix_names import ChangeNamedEntityNamesModule
from src.mglockne_story_line.story.event_sequence.modules.fictive_entities.entity_update2 import UpdateNamedEntityEntriesModule
from src.mglockne_story_line.story.event_sequence.modules.fictive_entities.find_new_entities_and_assign_ids import \
    FindNewAndOldEntitiesWithIDs
from src.mglockne_story_line.story.event_sequence.modules.fictive_entities.idfy_outline import IdfyOutlineWithNamedEntitiesModule
from src.mglockne_story_line.story.event_sequence.modules.improver.add_specifics import AddSpecificDetailsToOutlineModule
from src.mglockne_story_line.story.event_sequence.modules.improver.consistency_checker import CheckOutlineConsistencyModule
from src.mglockne_story_line.story.event_sequence.modules.improver.event_outline_refiner import OutlineRefiner
from src.mglockne_story_line.story.event_sequence.modules.recursive_outine_generator import OutlineGenerationModule
from src.mglockne_story_line.story.event_sequence.modules.story_alternative_continuations import \
    AlternativeStoryContinuationModule
from src.mglockne_story_line.story.event_sequence.modules.story_continuation import StoryContinuationModule
from src.mglockne_story_line.util.file_util import read_json, store_json, read_jsonl
...
#
#     components: List = [
#         OutlineGenerationModule(llm_write, 'A_outline-generate', 'v4', num_story_items),
#         AddSpecificDetailsToOutlineModule(llm_strict, 'B_outline-add-specifics', 'v6', add_specifics),
...
#                 ResolveFoundNamedEntityConflictsInOutlineModule(llm_strict, 'F_outline-fixnames', 'v3'),
...
#                 PopulateNewNamedEntitiesModule(llm_write, 'I_entities-fill-entries', 'v2'),
...
#         ),
#         StoryContinuationModule(llm_write, 'K_continue', 'v2-diverse', num_continuations),
#         AlternativeStoryContinuationModule(llm_write, 'L_continue_alt', 'v2', 3)
#     ]
#
...
#     pipeline: ModulePipeline = get_pipeline(llm_strict, llm_creative)
#
#     output_dir: str = join(OUTPUT_DIR, 'storylines-final')
#
...
# def generate_storylines(subset_key: str, claude_version: str = '3.5', max_num_events: int = 10, version=2):
#     llm_strict: BaseLLMWrapper = get_llm(claude_version, 0.0, 10000)
#     llm_creative: BaseLLMWrapper = get_llm(claude_version, 1.1, 10000)
#
#     for seed_item in get_seed_items(subset_key):
...
def main():
    ...
    if args['file']:
        summaries = read_jsonl(args['<file>'])
        summaries = sorted(summaries, key=lambda x: x['genre'])
        if args['<genre>']:
            summaries = [s for s in summaries if s['genre'].lower().replace(' ', '-') == args['<genre>']]
            assert len(summaries) == 1
        for summary_item in summaries:
            generate_storylines_for(summary_item)
            print(summary_item)
