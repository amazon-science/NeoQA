...
from data_gen.llm.modules.callable_module import FilePathModule
from data_gen.llm.modules.impl.file_output_caller import FileOutputCaller
from data_gen.llm.prompting.modules.shallow_xml_output_prompts import ShallowXMLListOutputPrompt
...
class EventCategorySeedGenerator(FileOutputCaller, FilePathModule):
    ...
    def call(self, news_genre: str, num_event_categories: int, print_results: bool = True):
        prompt: ShallowXMLListOutputPrompt = ShallowXMLListOutputPrompt(
            self.instructions, ['category'], self.instruction_name
        )
        out: Dict = self.query(
            prompt,
