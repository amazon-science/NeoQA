...
from src.mglockne_story_line.questions.question_gen_helper import get_outline_dict_for_events, get_xml_for_events, \
    get_xml_event, get_xml_event_selection
from src.mglockne_story_line.util.entity_util import get_xml_entities, get_prev_snapshot_entity_xml, \
...
class Newspaper:
    ...
    def create_news_articles(self, storyline: Dict, created_at: int, num_subsets: int, include_entity_updates: bool = False) -> Dict:
        ...
        sentence_dict: Dict[str, str] = {
            #item['id']: remove_ids_from(item['sentence']) for item in event['outline']
