...
from src.mglockne_story_line.questions.question_gen_helper import iterate_event_combinations, \
    get_outline_dict_for_events, get_selected_sentence_xml, get_xml_for_events
...
from src.mglockne_story_line.util.entity_util import entity_id_to_outline_items, entity_id_to_outline_items_from_events, \
    get_all_entity_names, get_prev_snapshot_entity_xml, get_entity_categories
...
class MultiEventMultiHopBridgeEntityQuestionGenerator(QuestionGenerator):
    ...
    def _iterate_storyline(self, storyline: Dict) -> Iterable[Dict]:
        ...
        for subset in iterate_event_combinations(storyline, self.include_single_events):
            ...
            if self.max_entities_per_selection is not None:
                random.Random(self.max_entity_per_selection_random_seed).shuffle(candidates)
                candidates = candidates[:self.max_entities_per_selection]
    ...
    #
    #
    # def _to_qa_pair(self, qa_data: Dict, validation_results: List[Dict], is_valid: bool, values: Dict) -> QAPair:
    #     distractors: List[Dict] = qa_data[QuestionGenerator.VAL_CURRENT_PAIR].get('distractors', [])
    #     return QAPair(
    ...
    #         num_hops=values[MultiEventTimeSpanQuestionGenerator.VAL_HOPS],
    #         is_valid=is_valid,
