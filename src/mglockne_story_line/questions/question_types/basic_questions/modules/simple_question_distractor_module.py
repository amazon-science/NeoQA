...
class SimpleQuestionDistractorModule(ParsableBaseModule):
    ...
    def __init__(self, llm: BaseLLMWrapper, name: str, instruction_name: str):
        ...
        raise NotImplementedError(f'todo: In the distractor, remember (if exists) the evidence IDs that make a distractor plausible.')
...
Your task is to create three plausible but incorrect multiple-choice distractor options for this question. These distractors should be challenging and appear realistic, but must not be valid answers to the question.
...
Present your three distractor options in the following format:
...
After each distractor, provide a brief explanation of why it's incorrect but plausible. This explanation should be included within the respective distractor tags.
