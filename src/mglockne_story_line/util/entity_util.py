...
def get_outline_dict_with_full_entity_names(outline: List[Dict], entity_snapshot: Dict) -> Dict[str, Dict]:
    ...
    for item in outline:
        ...
        if len(addon_information) > 0:
            ...
            decoded_sentence_corrected = remove_ids_from(item['sentence'])
            extra_details: str = '; '.join(sorted(list(addon_information)))
            decoded_sentence_corrected += f' ({extra_details})'
        else:
            decoded_sentence_corrected = decoded_sentence

        result[item['id']] = {
            'sentence': item['sentence'],
            'decoded_sentence': decoded_sentence,
            'decoded_sentence_corrected': decoded_sentence_corrected,
            'entity_ids': sorted(used_entities),
            'has_entity_name_conflict': decoded_sentence != decoded_sentence_corrected
        }
    return result
