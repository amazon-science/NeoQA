...
from src.mglockne_story_line.llm.verifiers.core_verifiers.cache.wiki_cache import WikiCache
...
def get_mean_max_popularity(start_date: str, end_date: str, entities: List[str]) -> Dict:

    if len(entities) == 0:
        raise ValueError('no elements!')

    mean_popularity_per_entity: List[float] = []
    max_popularity_per_entity: List[float] = []

    for entity in entities:
        entity_popularity: List[int] = [
            r['views'] for r in
            get_wikipedia_popularity(start_date, end_date, entity)
        ]

        mean_popularity_per_entity.append(float(np.mean(entity_popularity)))
        max_popularity_per_entity.append(int(np.max(entity_popularity)))

    return {
        'max_popularity': max(max_popularity_per_entity),
        'mean_popularity': max(mean_popularity_per_entity),  # We conservatively consider the mean of the most popular entity (if multiple match)
