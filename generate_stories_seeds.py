...
Usage:
  create-storylines.py seed-events
  create-storylines.py seed-summaries
  create-storylines.py extract1
  create-storylines.py extract2
  create-storylines.py extract-all
"""
import os
import random
import re
...
from src.mglockne_story_line.story.seeds.seed_event_category_generator import EventCategorySeedGenerator
from src.mglockne_story_line.story.seeds.seed_summary_generator import SeedSummaryGenerator

from src.mglockne_story_line.util.file_util import read_json, store_json, store_jsonl
...
def seed_events(genres: List[str], num_events_per_category: int, model_name: str) -> None:
    ...
    generator: EventCategorySeedGenerator = EventCategorySeedGenerator(
        # ClaudeWrapper(temperature=0.0, max_tokens=4000),
...
def seed_summaries(model_name: str, keywords: List[str] = None, num_summaries: int = 10):
    ...
    generator: SeedSummaryGenerator = SeedSummaryGenerator(
        #ClaudeWrapper(temperature=0.7, max_tokens=4000),
    ...
    for genre in genres:
        ...
        for event_type in event_types:
            ...
            for i, summary in enumerate(summaries):
                ...
                # We use this because story generation has some hard sampling. We want this to be seeded with different seeds
...
def is_okay(_summary):
    ...
    blacklist = [
        ...
        r'\btech\b', 'international', '-wide', 'blockchain', 'discovery', 'cryptocurrency', 'created a new' 'cyber', r'\bai\b' # we have this enough
    ]
...
def extract_summaries_all(random_seed: int, existing: List[str] = None):
    ...
    def count_items():
        return sum([
            len(genre_to_summaries[k]) for k in genre_to_summaries
        ])
...
def extract_summaries1(random_seed: int, out_name: str, exclude=None):
    directory: str = 'outputs/seed-summaries'
    files = [
        file for file in listdir(directory) if file.endswith('.json')
    ]
    ...
    genre_to_summaries = defaultdict(list)
    for file in files:
        summaries = read_json(join(directory, file))
        for event in summaries['events']:
            for summary in event['summaries']:
                if is_okay(summary):
                    genre_to_summaries[summary['genre']].append(summary)
    ...
    for key in genre_to_summaries:
        if key not in exclude:
            rnd.shuffle(genre_to_summaries[key])
            out.append(genre_to_summaries[key][0])

    for summary in out:
        print(summary['genre'], '-->', summary['summary'])

    exported_dir = join(directory, 'exported')
    os.makedirs(exported_dir, exist_ok=True)
    store_jsonl(out, join(exported_dir, out_name))
